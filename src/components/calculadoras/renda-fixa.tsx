        ...
        <div className="space-y-8 max-w-6xl">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-4 bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-card p-8 text-left">
                    ...
                    <button onClick={handleCalcular} className="w-full mt-8 h-14 bg-primary text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-primary-hover transition-all shadow-lg shadow-primary/20 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95">
                        Simular
                ...
                <div className="lg:col-span-8 flex flex-col gap-6 text-left">
                    {resultado ? (<>
                        <div className="bg-gradient-to-br from-primary to-blue-600 p-10 rounded-[2rem] shadow-xl relative overflow-hidden">
                            ...
                            <Landmark className="absolute bottom-6 right-8 w-24 h-24 text-white/10" />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-emerald-50 dark:bg-emerald-900/20 p-7 rounded-[2rem] border border-emerald-100 dark:border-emerald-800/30">
                                ...
                                <p className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mb-2">Rendimento Líquido</p>
                                <p className="text-2xl font-black text-emerald-700 dark:text-emerald-300 tabular-nums">{formatarMoeda(resultado.rendimentoLiquido)}</p>
                            </div>
                            {!resultado.isento && (
                                <div className="bg-rose-50 dark:bg-rose-900/20 p-7 rounded-[2rem] border border-rose-100 dark:border-rose-800/30">
                                    ...
                                    <p className="text-[10px] font-black text-rose-600 dark:text-rose-400 uppercase tracking-widest mb-2">IR Retido</p>
                                    <p className="text-2xl font-black text-rose-700 dark:text-rose-300 tabular-nums">{formatarMoeda(resultado.imposto)}</p>
                                </div>
                            )}
                        ...
                        <div className="bg-amber-50 dark:bg-amber-900/20 p-6 rounded-[2rem] border border-amber-100 dark:border-amber-800/30">
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    <h4 className="font-bold text-amber-800 dark:text-amber-200 mb-1">{tipo === 'cdb' ? 'CDB' : tipo === 'lci' ? 'LCI' : 'LCA'}</h4>
                                    <p className="text-sm text-amber-700 dark:text-amber-300/80 leading-relaxed">{tipo === 'cdb' ? 'Certificado de Depósito Bancário: tributado pela tabela regressiva de IR.' : 'Letras de Crédito: isentas de IR para pessoa física.'}</p>
                                </div>
                            </div>
                        </div>
                    </>) : (
